...
#---------------------------------------------------------------#
from data_prepro_upsampling import integrated_data_wrapper
import tensorflow as tf
from ln_models import lunar_mobilenet, lunar_net_basic, lunar_net, lunar_resnet, naive_inception_net
...
import pickle
from upsampling_main import full_procedure, just_bilinear
from keras.models import Model,load_model

call = CSVLogger('train.csv', separator=',', append=True)
checkpoint_path = 'newwrap_weights_43_mobile'
checkpoint_dir = os.path.dirname(checkpoint_path)
save_best = tf.keras.callbacks.ModelCheckpoint(
        filepath = os.path.join(checkpoint_path,
    '{epoch:02d}-besthigh.hdf5'),
        monitor = 'val_mean_absolute_error', 
        verbose = 1,
        mode = 'min', 
        save_best_only=True)
save_rounds = keras.callbacks.ModelCheckpoint(os.path.join(checkpoint_path,'{epoch:02d}-high.hdf5'), period=10, verbose=1)
...
def set_model(model_type,ex_w,ex_l,kernel_s,stride_l,num_filt,pool_s,model_name,
    ...
    elif model_type == 'lunar_mobilenet':
        model = lunar_mobilenet(ex_w, ex_l, num_layers, output_size)
    return model
...
def train_model(dg_ps, model_ps, train_ps):
    ...
    # get data
    ds = integrated_data_wrapper(*dg_ps) 
    mem('integrated data wrapper')
    x_t = ds.x_train
    y_t = ds.y_train
    x_v = ds.x_val
    y_v = ds.y_val
    ...
    if checkpoint_path_file is not None:
        ...
        history = model.fit(x=x_t, y=y_t, batch_size = train_ps[3],
            ...
            shuffle=False, steps_per_epoch=None,verbose=2, callbacks=[call,save_best,save_rounds], initial_epoch = max_value)
    else:
        print('no prev file')
        mem('before fit')
        print(x_t.shape)
        ...
        history = model.fit(x=x_t, y=y_t, batch_size = train_ps[3],
            epochs=train_ps[4], validation_data=(x_v, y_v),
            shuffle=False, steps_per_epoch=None,verbose=2, callbacks=[call,save_best, save_rounds])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_ups', type=int, default=0)
    ...
    save_dir = os.path.join(os.getcwd(), 'loss_graphs/') 
    ...
    margin = args.margin
    num_ups = args.num_ups
    
    vbs = args.vbs
    skip_amt = w//87 #args.skip_amt
    print(skip_amt)
    im_a = img_load(image_path)
    dg_ps = [im_a, w, l,  skip_amt, val_frac, vbs, margin, num_ups,  pred_type, just_bilinear] # x,y
    ...
    # train the model
    ln, ds, history = train_model(dg_ps, model_ps, train_ps)
    # evaluation
    plot_mse(str(w), history,  model_name,save_dir)
