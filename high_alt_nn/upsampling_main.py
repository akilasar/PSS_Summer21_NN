#------------------------------------------------#
# Upsampling with high/low split and		 #
# recombination					 #
# Author: Eric Ham				 #
#------------------------------------------------#
	...
	for i in range(im.shape[0] - 1):
		...
		for j in range(im.shape[1] - 1):
			...
			if j != 0 and i < im.shape[0] - 2: 
				#sub_out[:, 0] += left_col
				#sub_out[:,0] /= 2
				# minus 1 because you will be doing this with bottom row anyways below
...
if __name__ == '__main__':
	im = np.array(load_img('apollo113km.jpg', color_mode='grayscale'))
	#up_im = full_procedure(im, 4, 0.7, .7, 2, cr_spline, 25)
	#up_im = full_procedure(im, 4, 0.7, .7, 2, avg_test, 25)
	up_im = just_bilinear(im, 2)
	#up_im = just_diamond_square(im, 4, cr_spline, 25)
	#plt.imshow(up_im)
	#plt.show()
	save_dir ='up_justds.png'
