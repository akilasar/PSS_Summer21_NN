...
def im_train_val_split_with_skips(im_a, w, l, skip_amt, val_amt, vbs,
    ...
    print('train in: ',len(train_x_vals))
    print('train out: ',len(train_y_vals))
    print('val in: ',len(val_x_vals))
    print('val out: ',len(val_y_vals))
...
class data_set_wrapper:
    #@profile
    def __init__(self,im_a, w, l, val_frac, skip_amt, vbs, pred_type):
        if pred_type == 'xy':
...
if __name__ == '__main__':
    apollo_img_path ='apollo113km.jpg'
    ...
    a = data_set_wrapper(im_a, w, l, val_amt, skip_amt, vbs,'xy')
    del im_a
    gc.collect()
