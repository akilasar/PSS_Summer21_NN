...
from upsampling_main import just_bicubic, full_procedure, just_bilinear, just_diamond_square
...
# test point to img
def test_pts_to_img(t_x, t_y, t_samps):
...
class integrated_data_wrapper:
    ...
    def __init__(self,im_a, w, l, skip_amt, val_amt, vbs, margin, num_ups, pred_type, up_f, *up_args):
        ...
        # debugging test
        test_pts_to_img(t_x, t_y, t_samps)
...
if __name__ == '__main__':
    ...
    #for f in imgs:
        #im_a = Image.open(img_path+f+'.png')
        #w = im_a.width 
        #h = im_a.height
        #wdif, hdif = (w-cl)//2, (h-ch)//2
        #border = wdif, hdif, wdif, hdif  # left, top, right, bottom
        #cropped_img = ImageOps.crop(im_a, border)
        #cropped_img.save(img_path+"cropped"+f.split('_')[1]+'.png')

    im_a = img_load(img_path+'up2_120.png')
    w = 400
    l = 400
    skip_amt = 0
    val_amt = 0.2
